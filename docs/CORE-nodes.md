# The core, as nodes and edges

**A candidate replacement for `docs/CORE.md`, not a companion to it.** The two describe the
same instrument in incompatible shapes and only one can survive. Nothing cites this yet.

**What the format is.** Node, edge, source, act, the on-disk shape, the checks, the
operations. Written against the same test: can someone implement a reader from it alone?
